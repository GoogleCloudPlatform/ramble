...
class Spack(PackageManagerBase):
        ...
        try:
            ...
            for name, contents in workspace.all_auxiliary_software_files():
                aux_file_path = self.app_inst.expander.expand_var(
                    os.path.join(self.app_inst.expander.expansion_str(self.keywords.env_path), f"{name}")
                )
    ...
    def _define_package_paths(self, workspace, app_inst=None):
        ...
        software_environment = software_environments.render_environment(app_context, self.app_inst.expander)
        ...
        try:
            ...
            for pkg_spec in unresolved_specs:
                spack_pkg_name, pkg_path = self.runner.get_package_path(pkg_spec)
                if spack_pkg_name not in self.app_inst.variables:
                    self.app_inst.variables[spack_pkg_name] = pkg_path
        ...
        for var in variables:
            if isinstance(variables[var], str):
                variables[var] = variables[var].replace(
                    "\n".join(self.runner.generate_source_command()), "spack_source"
                )
                variables[var] = variables[var].replace(
                    "\n".join(self.runner.generate_activate_command()), "spack_activate"
                )
