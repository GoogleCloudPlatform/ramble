...
class SpackApplication(ApplicationBase):
    ...
    register_phase("make_experiments", pipeline="setup", run_after=["define_package_paths"])
    ...
    def build_used_variables(self, workspace):
        ...
        software_environments.render_environment(app_context, self.expander, require=False)

        return self.expander._used_variables

    register_phase(
        "software_install_requested_compilers", pipeline="setup", run_after=["software_create_env"]
    )
        ...
        try:
            ...
            for name, contents in workspace.all_auxiliary_software_files():
                aux_file_path = self.expander.expand_var(
                    os.path.join(self.expander.expansion_str(self.keywords.env_path), f"{name}")
                )
    ...
    register_phase("software_install", pipeline="setup", run_after=["software_configure"])
    ...
    register_phase("evaluate_requirements", pipeline="setup", run_after=["software_install"])
    ...
    def _define_package_paths(self, workspace, app_inst=None):
        ...
        app_context = self.expander.expand_var_name(self.keywords.env_name)
        software_environments = workspace.software_environments
        software_environment = software_environments.render_environment(app_context, self.expander)
        # Try to resolve using local cache first
        unresolved_specs = []
        for pkg_spec in software_environments.package_specs_for_environment(software_environment):
            if pkg_spec in cache:
                ...
                if spack_pkg_name not in self.variables:
                    ...
                    logger.msg(
                        f"Variable {spack_pkg_name} defined. " + "Skipping extraction from spack"
                    )
            else:
    ...
    def _clean_hash_variables(self, workspace, variables):
        ...
        for var in variables:
            if isinstance(variables[var], str):
                ...
                variables[var] = variables[var].replace(
                    "\n".join(self.spack_runner.generate_activate_command()), "spack_activate"
                )
    ...
    register_phase(
        "deploy_artifacts", pipeline="pushdeployment", run_after=["software_create_env"]
    )
